...
public:
    ...
    sampleWindow(HINSTANCE hInstance, int nCmdShow) :openglWindow(hInstance, nCmdShow) {
        ...
        textureImage("C:\\ccli\\resource\\image\\duckduck.jpeg");
    }
    bool textureImage(const char * pFile) {
        ...
        glBindTexture(GL_TEXTURE_2D, m_textureId);//将texID绑定  意味着之后的GL_TEXTURE_2D操作，都是对texId进行
        // 前三个参数  纹理的类型。
